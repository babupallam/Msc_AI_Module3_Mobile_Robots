...
class Robot():
    ...
    def __init__(self):
        ...
        res, self.frontLeftSonar = vrep.simxGetObjectHandle(clientID, 'Pioneer_p3dx_ultrasonicSensor6',
                                                            vrep.simx_opmode_blocking)
        res, self.frontRightSonar = vrep.simxGetObjectHandle(clientID, 'Pioneer_p3dx_ultrasonicSensor1',
                                                             vrep.simx_opmode_blocking)
        res, self.sideSonar = vrep.simxGetObjectHandle(clientID, 'Pioneer_p3dx_ultrasonicSensor0',
                                                       vrep.simx_opmode_blocking)
        res, self.rightSonar = vrep.simxGetObjectHandle(clientID, 'Pioneer_p3dx_ultrasonicSensor15',
                                                        vrep.simx_opmode_blocking)
        ...
        res, detectionState, detectedPoint, detectedObjectHandle, detectedSurfaceNormalVector = vrep.simxReadProximitySensor(
            clientID, self.frontLeftSonar, vrep.simx_opmode_streaming)
        res, detectionState, detectedPoint, detectedObjectHandle, detectedSurfaceNormalVector = vrep.simxReadProximitySensor(
            clientID, self.frontRightSonar, vrep.simx_opmode_streaming)
        res, detectionState, detectedPoint, detectedObjectHandle, detectedSurfaceNormalVector = vrep.simxReadProximitySensor(
            clientID, self.rightSonar, vrep.simx_opmode_streaming)
        res, detectionState, detectedPoint, detectedObjectHandle, detectedSurfaceNormalVector = vrep.simxReadProximitySensor(
            clientID, self.sideSonar, vrep.simx_opmode_streaming)
    ...
    def getDistanceReading(self, objectHandle):
        ...
        res, detectionState, detectedPoint, detectedObjectHandle, detectedSurfaceNormalVector = vrep.simxReadProximitySensor(
            clientID, objectHandle, vrep.simx_opmode_buffer)
...
if clientID != -1:
    ...
    while True:
        ...
        res, detectionStateR, detectedPoint, detectedObjectHandle, detectedSurfaceNormalVector = vrep.simxReadProximitySensor(
            clientID, robot.rightSonar, vrep.simx_opmode_buffer)
        ...
        elif robot.getDistanceReading(robot.rightSonar) == dist_to_wall and robot.getDistanceReading(
                robot.sideSonar) == dist_to_wall and detectionStateR == 1:  # threshold value
        ...
        elif robot.getDistanceReading(
                robot.frontRightSonar) <= dist_to_wall and detectionStateR == 1:  # threshold value
            robot.turnArc(cp, angle)  # velocity adjustment
            print("Change direction to the Right")
            # the robot will turn if object detected on the right within the threshold value;
        ...
        elif robot.getDistanceReading(robot.sideSonar) == dist_to_wall and robot.getDistanceReading(
                robot.rightSonar) == dist_to_wall and detectionStateR == 1:  # threshold value
            robot.move(1)  # velocity adjustment
            print("Forward Maintain Velocity")
            # the robot will turn to adjust direction;
